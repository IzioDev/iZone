			...
			<div class="display">
						...
						<table class='center'>
							...
							<tbody>
								...
								<tr>
									...
									<td>
										x: {Math.ceil(zone.center.x)} y: {Math.ceil(zone.center.y)} z: {Math.ceil(zone.center.z)}
									</td>
									<td>
										<i on:click={tpToSelected} id="{i}" class="material-icons center clickable md-48">play_arrow</i>
									</td>
									<td>
										<i on:click={() => deleteRequested(zone)} id="{zone.id}" class="material-icons center red clickable md-48">delete_forever</i>
									</td>
								</tr>
								{/each}
							</tbody>
				...
				{#if currentDisp == "help"}
					<div class="center container"> 
						<h2>Help</h2>
            <ul class="help-list">
              <li>Create a Zone: click on the <span class='orange'>Create Zone</span> button.</li>
              <li>Add points: press the <span class='orange'>[L]</span> key (the keys can be changed in configuration file).</li>
              <li>Remove last point: press the <span class='orange'>[K]</span> key.</li>
              <li>Save the zone: click on the <span class='orange'>Save Zone</span> button, a prompt will ask you to enter a categorie name and a zone name.</li>
              <li>Stop the zone creation: click on the <span class='orange'>Stop Zone Creation</span></li>
            </ul>
					</div>
				{/if}
				...
				{#if currentDisp == "about"}
					<div class="center container"> 
						<h2>About</h2>
						<p><span class='orange'>iZone</span> is made by Izio to help developer and server owners to create polygonal 2D zones. It also provides few utilities to interract with the created zones.</p>
						<p>If you want to <span class='orange'>contribute</span> to this project, go to the CitizenFx forum, and follow the GitHub link.</p>
						<p>Feel free to <span class='orange'>report the bugs</span> or <span class='orange'>suggest new features</span>!</p>
					</div>
				{/if}
			</div>
			<!-- Hello {name}! -->
